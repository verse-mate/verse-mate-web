 ...
 *  - automatic pageview tracking (manual on SPA route change, since
 *    posthog-js default `capture_pageview` only fires on full reloads)
 *  - global error handlers (window.onerror + unhandledrejection) with
 *    the same filters as the monorepo's frontend-base analytics
...
export function PostHogProvider({ children }: PostHogProviderProps) {
  ...
  useEffect(() => {
    ...
    if (!POSTHOG_KEY) {
      // Anonymous dev builds — skip without error.
      return;
    }
  ...
  return (
    <PHProvider client={posthog}>
      <PageviewTracker />
      <ErrorHandlerInstaller
        errorHandlersSet={errorHandlersSet}
      />
      {children}
    </PHProvider>
  );
...
/**
 * Captures a $pageview event on every React Router location change.
 * Mounted inside the BrowserRouter so useLocation() works.
 */
