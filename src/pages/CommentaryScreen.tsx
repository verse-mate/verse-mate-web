...
/**
 * CommentaryScreen — dark header with text pill tabs (Insight active/gold, Bible inactive/white),
 * tab pills container with #323232 bg, pure black body area.
 */
export default function CommentaryScreen() {
  ...
  return (
    <div className="flex flex-col h-full text-white" style={{ backgroundColor: '#1B1B1B' }}>
      {/* Header — #1A1A1A, Insight pill gold (active), Bible pill white (inactive) */}
      <header
      ...
      {/* Body — pure black background, white text */}
                  ...
                  {byLineItems.map(c => {
                    ...
                    return (
                      <div key={c.verse} style={{ borderBottom: '1px solid hsl(var(--dark-border))' }}>
                        ...
                        >
                          <span style={{ fontFamily: 'Roboto, sans-serif', fontSize: 15, color: '#fff' }}>
...
function CommentaryBody({ text }: { text: string }) {
  ...
  const flushPara = () => {
    if (para.length) {
      elements.push(
        <p key={key++} style={{ fontFamily: 'Roboto, sans-serif', fontWeight: 400, fontSize: 16, lineHeight: '24px', color: '#fff', marginBottom: 12 }}>
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith('###') || line.startsWith('##') || line.startsWith('#')) {
      ...
      elements.push(
        <h2 key={key++} style={{ fontFamily: 'Roboto, sans-serif', fontWeight: 500, fontSize: 18, lineHeight: '24px', color: '#fff', marginTop: 16, marginBottom: 8 }}>
          {inlineFormat(text)}
        </h2>
      );
      continue;
    ...
    if (line.startsWith('>')) {
      flushPara();
      elements.push(
        <blockquote key={key++} style={{ borderLeft: '2px solid #b09a6d', paddingLeft: 12, fontStyle: 'italic', marginBottom: 12, fontFamily: 'Roboto, sans-serif', fontSize: 16, lineHeight: '24px', color: 'rgba(255,255,255,0.8)' }}>
