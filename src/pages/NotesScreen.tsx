              ...
              {groups.map(g => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <FileText size={18} className="text-dark-fg" strokeWidth={1.75} />
                    <span className="text-[15px] text-dark-fg">
                      {g.book} {g.chapter}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[12px] text-dark-muted bg-dark-surface rounded px-2 py-0.5 min-w-[28px] text-center">
                      {g.count}
                    </span>
    ...
    <div className="flex flex-col h-full bg-dark-surface text-dark-fg relative">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-2 pb-6">
        {notesForChapter.length === 0 ? (
          <p className="text-center text-dark-muted py-8 text-[14px]">No notes here yet.</p>
        ) : (
          <div className="space-y-3">
            {notesForChapter.map(note => (
              <button
                ...
                className="flex items-center justify-between w-full min-h-[56px] px-4 rounded-xl bg-dark-raised border border-dark text-left"
              >
                <span className="text-[14px] text-dark-fg pr-3 line-clamp-2">
